...
| Savings Plans / Reserved Instances | 稳定长期使用，以承诺换取折扣 |
| Spot | 可中断、容错的任务，价格更低 |
| Dedicated Instance | 需要单租户硬件，但不控制具体主机 |
| Dedicated Host | 需要物理主机可见性或许可证合规 |
...
- 实例可以按需创建、停止、调整或删除。
- EC2 经常与 EBS、ELB 和 Auto Scaling 配合使用。

## SAA-C03 考点

- 根据 CPU、内存、存储和网络需求选择实例系列。
- 使用 AMI 和 Launch Template 标准化部署。
- 使用多 AZ、ELB 和 Auto Scaling 提高可用性与弹性。
- 按可中断性和使用稳定度选择计费方式。
- 使用 IAM Role 代替应用中的长期访问密钥。
...
- Dedicated Host 和 Dedicated Instance 并不相同。

## 面试高频问题

1. EC2、Lambda、Fargate 如何选择？
2. 如何设计跨 AZ 的 EC2 高可用架构？
3. Spot 实例适合什么任务，如何处理被回收？
4. IAM Role、Security Group、EBS 和 AMI 分别负责什么？

## 重点记忆

> EC2 = 控制权高，但管理责任也高。

经典高可用组合：**AMI / Launch Template + Multi-AZ ASG + ELB + CloudWatch**。
